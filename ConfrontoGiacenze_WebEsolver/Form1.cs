...
namespace ConfrontoGiacenze_WebEsolver
{
    public partial class Form1 : Form
    {
        ...
        public string ConnessioneDBEsolver()
        {
            string pathINI = Directory.GetCurrentDirectory() + "\\ConfrontoGiacenzeINI.ini";

            INIFile fileini = new INIFile(pathINI);

            String connEsolver = "Server = '" +fileini.IniReadValue("dstsm", "Sezione1")+ "'; Database ='" +fileini.IniReadValue("cattsm", "Sezione1")+"'; User Id = '"+fileini.IniReadValue("usertsm", "Sezione1")+ "'; Password = '"+ fileini.IniReadValue("pwdtsm", "Sezione1")+"';";

            //SqlConnection conn = new SqlConnection(connEsolver);

            return connEsolver;

        }

        public SqlConnection ConnessioneDBWeb()
        {
            string pathINI = Directory.GetCurrentDirectory() + "\\ConfrontoGiacenzeINI.ini";

            INIFile fileini = new INIFile(pathINI);

            String connWeb = "Server = '" + fileini.IniReadValue("dsweb", "Sezione1") + "'; Database ='" + fileini.IniReadValue("catweb", "Sezione1") + "'; User Id = '" + fileini.IniReadValue("userweb", "Sezione1") + "'; Password = '" + fileini.IniReadValue("pwdweb", "Sezione1") + "';";

            SqlConnection conn2 = new SqlConnection(connWeb);

            return conn2;

        }

        public string QueryGiacenzeWeb()
        {
            String query = @"SELECT CodiceArticolo, Varianti.Codice, CodiceMagazzino, Quantita1, Quantita2, Quantita3, Quantita4, Quantita5, Quantita6, Quantita7, Quantita8, Quantita9, Quantita10,
...
ORDER BY CodiceArticolo, Codice, CodiceMagazzino  ";
        ...
        public int[] GiacenzeEsolver(string Articolo, string Variante, string Magazzino)
        {
            int[] GiacenzeEsolver = new int[30];
            ...
            SQL = SQL + " WHERE OrdSpedTestata.StatoSpedizione < 4 And CodArt = '" + Articolo + "' AND VarianteArt = '" + Variante + "' AND CodMag = '" + Magazzino + "'";


            using (SqlConnection esolver = new SqlConnection(ConnessioneDBEsolver()))
            {
                esolver.Open();
                ...
                giacMag.Parameters["@ARTICOLO"].Value = Articolo;

                giacMag.Parameters.Add("@VARIANTE", SqlDbType.VarChar);
                giacMag.Parameters["@VARIANTE"].Value = Variante;

                giacMag.Parameters.Add("@MAGAZZINO", SqlDbType.VarChar);
                giacMag.Parameters["@MAGAZZINO"].Value = Magazzino;
                ...
                if (GiacNetta.HasRows)
                {
                    ...
                    for(int i=1; i<=30; i++)
                    {
                        if (GiacNetta["GtgQgp" + i.ToString()] != DBNull.Value)
                        {
                            ...
                            {
                                GiacenzeEsolver[i - 1] = Convert.ToInt32(GiacNetta["GtgQgp" + i.ToString()]);
                            }
                            else { GiacenzeEsolver[i - 1] = 0; }
                        }
                        else
                        { GiacenzeEsolver[i - 1] = 0; }
                    }
                }
...
//            MsgAddLog("finito il primo inizio con il secondo");
                ...
                if (Prelevato.HasRows)
                {
                    ...
                    for (int i = 1; i <= 30; i++)
                    {
                        if (Prelevato["QtaTagDaSpedire_" + i.ToString()] != DBNull.Value)
                        {
                            ...
                            {
                                GiacenzeEsolver[i - 1] = GiacenzeEsolver[i - 1] - Convert.ToInt32(Prelevato["QtaTagDaSpedire_" + i.ToString()]);

                                if(GiacenzeEsolver[i - 1] < 0)
                                {
                                    GiacenzeEsolver[i - 1] = 0;
                                }
                            }
        ...
        private void Form1_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                String sqlWeb = QueryGiacenzeWeb();
                ...
                if (myReader.HasRows)
                {
                    while (myReader.Read())
                    {
                        ...
                        int[] giacEsolver = GiacenzeEsolver(myReader["CodiceArticolo"].ToString(), myReader["Codice"].ToString(), myReader["CodiceMagazzino"].ToString());
